...
export default function LotItem({ lot }: DetailLotItemProps) {
    ...
    useEffect(() => {
        ...
        const updateCountdown = () => {
            const endTime = new Date(lot.expectedEndTime).getTime();
    ...
    return (
        <TouchableOpacity className="m-3 p-[10] w-[180] h-[350] bg-white rounded-[10]" onPress={() => router.push(`/(components)/LotDetailScreen?lotId=${lot?.id}`)}>
            ...
            <View className="p-[7] flex gap-[3]">
                ...
                <Text>
                    Sex: {lot?.gender || 'Unknown'}
                </Text>
                <Text>
                    Size: {lot?.size ? `${lot.size} cm` : 'Unknown'}
                </Text>
